...
#pragma GCC diagnostic pop
  ...
  bool initProgram()
  {
    ...
    CHECK_CL_RETURN(kernel_prepareIrShadowDiscard.setArg(0, config.EnableEdgeAwareFilter ? buf_filtered : buf_depth));
    ...
    CHECK_CL_RETURN(kernel_applyIrShadowDiscard.setArg(0, config.EnableEdgeAwareFilter ? buf_filtered : buf_depth));
  ...
  {
    source = loadCLSource("opencl_depth_packet_processor.cl") + loadCLSource("opencl_depth_shadow_removal.cl");
    return !source.empty();
  }
